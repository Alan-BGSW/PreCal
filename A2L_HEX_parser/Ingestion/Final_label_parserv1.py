...
def _parse_calibration_row(
    ...
    context = _extract_row_base_context_verbatim(label, row_lines_raw or row_lines)
...
def parse_document(document_path: Path, max_row_lines: int = 20) -> tuple[list[CalibrationRecord], list[ValueTableRecord]]:
    ...
    for page_no, page_text in pages:
        ...
        for start, end in sections:
            ...
            pending_continuation_raw: list[str] = []
            ...
            while cursor < end:
                ...
                if not label:
                    if section_tail_record is not None:
                        pending_continuation_raw.append(lines[cursor])
                    cursor += 1
                    continue

                if pending_continuation_raw and section_tail_record is not None:
                    _merge_unlabeled_continuation_into_record(section_tail_record, pending_continuation_raw)
                    pending_continuation_raw = []
